...
### 4.1 实验性：半自动投递 `autopilot`

想进一步减少重复操作，可用实验性的半自动模式：

```bash
python -m eresume autopilot                      # 人工协助：开浏览器→给话术→逐条确认→记录
python -m eresume autopilot --auto               # 自动模式（需 pip install selenium）
```

**使用前会先弹出风险告知，输入「我了解风险」才继续**，内容包括：

- 自动化操作违反平台 ToS，可能**封号**
- **自动回复 HR 绝不代发**——本工具只生成草稿，发送永远由你确认后手动完成
- 平台改版会导致自动模式失效，失效会**自动回退到人工协助模式**，不强行重试

> 建议：仅个人使用、低频小批量、每条投递逐条确认。封号风险自担。
